...
func TestStartPendingStep_LineContainsLabel(t *testing.T) {
	...
	if !strings.Contains(m.commandsBuf[0], "Skaffold") {
		t.Fatalf("expected Skaffold in line, got %q", m.commandsBuf[0])
	}
}
...
func TestStartPendingStep_LineContainsDep(t *testing.T) {
	m := newStepsModel()
	m.startPendingStep("skaffold", "Skaffold", []string{"minikube"})
	if !strings.Contains(m.commandsBuf[0], "minikube") {
		t.Fatalf("expected minikube in line, got %q", m.commandsBuf[0])
	}
}
...
func TestStartPendingStep_LineContainsWaitingFor(t *testing.T) {
	m := newStepsModel()
	m.startPendingStep("skaffold", "Skaffold", []string{"minikube"})
	if !strings.Contains(m.commandsBuf[0], "waiting for") {
		t.Fatalf("expected 'waiting for' in line, got %q", m.commandsBuf[0])
	}
}
...
func TestStartPendingStep_DoesNotMutateDepsSlice(t *testing.T) {
	...
	deps[0] = "mutated"
	if m.steps["app"].pendingDeps[0] == "mutated" {
...
func TestDepReady_RemovesCompletedDep(t *testing.T) {
	...
	if strings.Contains(m.commandsBuf[0], "infra") {
		t.Fatalf("expected infra removed from line, got %q", m.commandsBuf[0])
	}
}
...
func TestDepReady_PreservesRemainingDep(t *testing.T) {
	...
	m.startPendingStep("app", "App", []string{"infra", "config"})
	m.depReady("app", "infra")
	if !strings.Contains(m.commandsBuf[0], "config") {
		t.Fatalf("expected config to remain in line, got %q", m.commandsBuf[0])
	}
}
...
func TestDepReady_StillShowsWaitingForRemaining(t *testing.T) {
	...
	m.depReady("app", "infra")
	if !strings.Contains(m.commandsBuf[0], "waiting for") {
		t.Fatalf("expected still waiting for config, got %q", m.commandsBuf[0])
	}
}
...
func TestDepReady_ClearsWaitingSuffixWhenLastDepDone(t *testing.T) {
	...
	m.startPendingStep("app", "App", []string{"infra"})
	m.depReady("app", "infra")
	if strings.Contains(m.commandsBuf[0], "waiting") {
		t.Fatalf("expected no waiting suffix after last dep, got %q", m.commandsBuf[0])
	}
}
...
func TestDepReady_PreservesLabelAfterAllDone(t *testing.T) {
	...
	m.depReady("app", "infra")
	if !strings.Contains(m.commandsBuf[0], "My App") {
		t.Fatalf("expected base label preserved, got %q", m.commandsBuf[0])
	}
}
...
func TestDepReady_SequentialCompletions(t *testing.T) {
	...
	m.depReady("app", "a")
	if strings.Contains(m.commandsBuf[0], "\"a\"") || !strings.Contains(m.commandsBuf[0], "b") {
		t.Fatalf("after removing a, expected b and c in line, got %q", m.commandsBuf[0])
	}
	m.depReady("app", "b")
	if strings.Contains(m.commandsBuf[0], "b") || !strings.Contains(m.commandsBuf[0], "c") {
		t.Fatalf("after removing b, expected only c in line, got %q", m.commandsBuf[0])
	}
	m.depReady("app", "c")
	if strings.Contains(m.commandsBuf[0], "waiting") {
		t.Fatalf("after all deps done, expected no waiting suffix, got %q", m.commandsBuf[0])
	}
}
...
func TestDepReady_UnknownDepLeavesLineUnchanged(t *testing.T) {
	m := newStepsModel()
	m.startPendingStep("app", "App", []string{"infra"})
	before := m.commandsBuf[0]
	m.depReady("app", "no-such-dep")
	if m.commandsBuf[0] != before {
		t.Fatalf("expected line unchanged for unknown dep\nbefore: %q\nafter:  %q", before, m.commandsBuf[0])
	}
}
...
func TestDepReady_MultipleStepsIndependent(t *testing.T) {
	...
	if !strings.Contains(m.commandsBuf[1], "infra") {
		t.Fatalf("worker line should still contain infra, got %q", m.commandsBuf[1])
	}
}
